    ...
    public class GaussianSplatAssetCreator : EditorWindow
    {
        ...
        unsafe void CreateAsset()
        {
            ...
            asset.SetAssetFiles(
                ...
                AssetDatabase.LoadAssetAtPath<TextAsset>(pathTemporal));
        ...
        struct SplatTemporalData
        {
            // 13 floats per splat
            public float t;
            public float scale_t;
            public float rot_r_x, rot_r_y, rot_r_z, rot_r_w;  // rot_r quaternion
            public float rot_l_x, rot_l_y, rot_l_z, rot_l_w;  // rot (left) quaternion raw
            public float scale_x, scale_y, scale_z;            // scale raw (log space)
        }

        void CreateTemporalData(NativeArray<InputSplatData> inputSplats, string filePath, ref Hash128 dataHash)
        {
            int count = inputSplats.Length;
            NativeArray<SplatTemporalData> data = new NativeArray<SplatTemporalData>(count, Allocator.TempJob);
            for (int i = 0; i < count; i++)
            {
                var s = inputSplats[i];
                data[i] = new SplatTemporalData
                {
                    t       = s.t,
                    scale_t = s.scale_t,
                    rot_r_x = s.rot_r.x,
                    ...
                    rot_r_z = s.rot_r.z,
                    rot_r_w = s.rot_r.w,
                    rot_l_x = s.rot_raw_val.x,
                    rot_l_y = s.rot_raw_val.y,
                    rot_l_z = s.rot_raw_val.z,
                    rot_l_w = s.rot_raw_val.w,
                    scale_x = math.log(s.scale.x),
                    scale_y = math.log(s.scale.y),
                    scale_z = math.log(s.scale.z),
                };
            }
            EmitSimpleDataFile(data, filePath, ref dataHash);
            data.Dispose();
