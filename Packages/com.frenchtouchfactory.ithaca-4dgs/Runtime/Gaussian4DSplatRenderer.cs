...
{
    class Gaussian4DSplatRenderSystem
    {
        ...
        public static Gaussian4DSplatRenderSystem instance => ms_Instance ??= new Gaussian4DSplatRenderSystem();
        static Gaussian4DSplatRenderSystem ms_Instance;

        readonly Dictionary<Gaussian4DSplatRenderer, MaterialPropertyBlock> m_Splats = new();
        readonly HashSet<Camera> m_CameraCommandBuffersDone = new();
        readonly List<(Gaussian4DSplatRenderer, MaterialPropertyBlock)> m_ActiveSplats = new();

        CommandBuffer m_CommandBuffer;

        public void RegisterSplat(Gaussian4DSplatRenderer r)
        {
            if (m_Splats.Count == 0)
            {
        ...
        public void UnregisterSplat(Gaussian4DSplatRenderer r)
        {
        ...
        public Material SortAndRenderSplats(Camera cam, CommandBuffer cmb)
        {
            ...
            foreach (var kvp in m_ActiveSplats)
            {
                ...
                {
                    Gaussian4DSplatRenderer.RenderMode.DebugPoints => gs.m_MatDebugPoints,
                    Gaussian4DSplatRenderer.RenderMode.DebugPointIndices => gs.m_MatDebugPoints,
                    Gaussian4DSplatRenderer.RenderMode.DebugBoxes => gs.m_MatDebugBoxes,
                    Gaussian4DSplatRenderer.RenderMode.DebugChunkBounds => gs.m_MatDebugBoxes,
                    _ => gs.m_MatSplats
                };
                if (displayMat == null)
                    continue;
                ...
                mpb.SetBuffer(Gaussian4DSplatRenderer.Props.SplatChunks, gs.m_GpuChunks);

                mpb.SetBuffer(Gaussian4DSplatRenderer.Props.SplatViewData, gs.m_GpuView);

                mpb.SetBuffer(Gaussian4DSplatRenderer.Props.OrderBuffer, gs.m_GpuSortKeys);
                mpb.SetFloat(Gaussian4DSplatRenderer.Props.SplatScale, gs.m_SplatScale);
                mpb.SetFloat(Gaussian4DSplatRenderer.Props.SplatOpacityScale, gs.m_OpacityScale);
                mpb.SetFloat(Gaussian4DSplatRenderer.Props.SplatSize, gs.m_PointDisplaySize);
                mpb.SetInteger(Gaussian4DSplatRenderer.Props.SHOrder, gs.m_SHOrder);
                mpb.SetInteger(Gaussian4DSplatRenderer.Props.SHOnly, gs.m_SHOnly ? 1 : 0);
                mpb.SetInteger(Gaussian4DSplatRenderer.Props.DisplayIndex, gs.m_RenderMode == Gaussian4DSplatRenderer.RenderMode.DebugPointIndices ? 1 : 0);
                mpb.SetInteger(Gaussian4DSplatRenderer.Props.DisplayChunks, gs.m_RenderMode == Gaussian4DSplatRenderer.RenderMode.DebugChunkBounds ? 1 : 0);
                ...
                if (gs.m_RenderMode is Gaussian4DSplatRenderer.RenderMode.DebugBoxes or Gaussian4DSplatRenderer.RenderMode.DebugChunkBounds)
                    indexCount = 36;
                if (gs.m_RenderMode == Gaussian4DSplatRenderer.RenderMode.DebugChunkBounds)
        ...
        void OnPreCullCamera(Camera cam)
        {
            ...
            m_CommandBuffer.GetTemporaryRT(Gaussian4DSplatRenderer.Props.GaussianSplatRT, -1, -1, 0, FilterMode.Point, GraphicsFormat.R16G16B16A16_SFloat);
            m_CommandBuffer.SetRenderTarget(Gaussian4DSplatRenderer.Props.GaussianSplatRT, BuiltinRenderTextureType.CurrentActive);
            ...
            m_CommandBuffer.SetGlobalTexture(Gaussian4DSplatRenderer.Props.CameraTargetTexture, BuiltinRenderTextureType.CameraTarget);
            ...
            m_CommandBuffer.ReleaseTemporaryRT(Gaussian4DSplatRenderer.Props.GaussianSplatRT);
        }
    }

    [ExecuteInEditMode]
    public class Gaussian4DSplatRenderer : MonoBehaviour
    {
        ...
        public float m_MaxSplatSize = 0f;
        [HideInInspector] public float m_MinSplatOpacity = 0f;
        [HideInInspector] public float m_TemporalCutoff = 0.05f;
        public float m_Is4D = 0f;
        bool m_CSInstanced = false;
        [Range(0.05f, 20.0f)]
        ...
        GraphicsBuffer m_GpuPosDisplaced;
        int m_PosDisplacedReady;
        ...
        public GraphicsBuffer m_GpuTemporalDummy;
        [HideInInspector] public float m_TemporalCurrentTime = 0f;
        ...
        internal static class Props
        {
            ...
            public static readonly int MaxSplatSize = Shader.PropertyToID("_MaxSplatSize");
            public static readonly int MinSplatOpacity = Shader.PropertyToID("_MinSplatOpacity");
            public static readonly int TemporalCutoff = Shader.PropertyToID("_TemporalCutoff");
            public static readonly int Is4D = Shader.PropertyToID("_Is4D");
            ...
            public static readonly int SplatPosDisplaced = Shader.PropertyToID("_SplatPosDisplaced");
            public static readonly int PosDisplacedReady = Shader.PropertyToID("_PosDisplacedReady");
        ...
        void CreateResourcesForAsset()
        {
            ...
            m_Is4D = 0f; // reset, le player 4D le remet a 1 si present
            ...
            // Dummy temporal buffer (replaced by Gaussian4DPlayer at runtime)
            m_GpuTemporalDummy = new GraphicsBuffer(GraphicsBuffer.Target.Raw, 6, 4) { name = "GaussianTemporalDummy" };
            m_GpuTemporalDummy.SetData(new uint[6]);
        ...
        void InitSortBuffers(int count)
        {
            ...
            m_GpuPosDisplaced?.Dispose();
            m_PosDisplacedReady = 0;
            ...
            m_GpuPosDisplaced = new GraphicsBuffer(GraphicsBuffer.Target.Structured, count, 12) { name = "GaussianSplatPosDisplaced" };

            // init keys buffer to splat indices - via CommandBuffer local pour garantir binding+dispatch atomique
            // (sinon, CS partage entre plusieurs renderers => course sur le buffer de sort => streaks/non-deterministe)
            m_CSSplatUtilities.GetKernelThreadGroupSizes((int)KernelIndices.SetIndices, out uint gsX, out _, out _);
            {
                var initCmb = new CommandBuffer { name = "GaussianInitSortKeys" };
                initCmb.SetComputeBufferParam(m_CSSplatUtilities, (int)KernelIndices.SetIndices, Props.SplatSortKeys, m_GpuSortKeys);
                initCmb.SetComputeIntParam(m_CSSplatUtilities, Props.SplatCount, m_GpuSortDistances.count);
                initCmb.DispatchCompute(m_CSSplatUtilities, (int)KernelIndices.SetIndices, (m_GpuSortDistances.count + (int)gsX - 1)/(int)gsX, 1, 1);
                Graphics.ExecuteCommandBuffer(initCmb);
                initCmb.Release();
            }
        ...
        public void EnsureSorterAndRegister()
        {
            ...
            {
                Gaussian4DSplatRenderSystem.instance.RegisterSplat(this);
                m_Registered = true;
            }
        }
        ...
        {
#if UNITY_EDITOR
            m_ShaderSplats = FindAssetByName<Shader>("RenderGaussianSplats");
            m_ShaderComposite = FindAssetByName<Shader>("GaussianComposite");
            m_ShaderDebugPoints = FindAssetByName<Shader>("GaussianDebugRenderPoints");
            m_ShaderDebugBoxes = FindAssetByName<Shader>("GaussianDebugRenderBoxes");
            m_CSSplatUtilities = FindAssetByName<ComputeShader>("Splat4DUtilities");
#endif
        }

#if UNITY_EDITOR
        // Auto-assignation des ressources (shaders + compute shader) a l'ajout du composant,
        // par recherche de nom dans AssetDatabase -- evite l'assignation manuelle des 5 champs
        // a chaque nouvelle installation du package (gain de temps).
        static T FindAssetByName<T>(string name) where T : UnityEngine.Object
        {
        ...
        public void OnEnable()
        {
            ...
            // m_PrevAsset/m_PrevHash ne sont normalement mis a jour que dans Update() lui-meme (pour
            // detecter un changement d'asset EN COURS DE ROUTE) -- sans cette initialisation ici, le
            // tout premier Update() apres CHAQUE OnEnable() les trouve encore a leur valeur par defaut,
            // differente de l'asset reel, et redeclenche a tort un second Dispose+CreateResourcesForAsset
            // qui reset m_Is4D et peut invalider un buffer temporel deja assigne par Gaussian4DPlayer.
            m_PrevAsset = m_Asset;
            m_PrevHash = m_Asset ? m_Asset.dataHash : new Hash128();
        }

        // Garantit que _SplatTemporal ne soit jamais bind sur une reference null : au tout
        // premier frame de Play, Gaussian4DPlayer.OnEnable() peut s'executer avant ou apres
        // Renderer.OnEnable() selon l'ordre de script Unity (non garanti sans configuration
        // explicite) -- dans le pire cas, m_GpuTemporalDummy pourrait etre momentanement null
        // entre la creation des ressources et la premiere reassignation par le player. On
        // recree ici un petit buffer factice de secours (identique a celui de
        // CreateResourcesForAsset()) plutot que de laisser Unity logger une erreur de binding.
        GraphicsBuffer EnsureTemporalBuffer()
        {
            if (m_GpuTemporalDummy == null)
            {
                m_GpuTemporalDummy = new GraphicsBuffer(GraphicsBuffer.Target.Raw, 6, 4) { name = "GaussianTemporalDummy" };
                m_GpuTemporalDummy.SetData(new uint[6]);
            }
            return m_GpuTemporalDummy;
        }
        ...
        void SetAssetDataOnCS(CommandBuffer cmb, KernelIndices kernel)
        {
            ...
            cmb.SetComputeBufferParam(cs, kernelIndex, Shader.PropertyToID("_SplatTemporal"), EnsureTemporalBuffer());
            cmb.SetComputeFloatParam(cs, Shader.PropertyToID("_SplatCurrentTime"), m_TemporalCurrentTime);
        ...
        internal void SetAssetDataOnMaterial(MaterialPropertyBlock mat)
        {
            ...
            mat.SetBuffer(Shader.PropertyToID("_SplatTemporal"), EnsureTemporalBuffer());
            mat.SetFloat(Shader.PropertyToID("_SplatCurrentTime"), m_TemporalCurrentTime);
        ...
        void DisposeResourcesForAsset()
        {
            ...
            DisposeBuffer(ref m_GpuTemporalDummy);
            DisposeBuffer(ref m_GpuIndexBuffer);
            DisposeBuffer(ref m_GpuSortDistances);
            DisposeBuffer(ref m_GpuSortKeys);
            DisposeBuffer(ref m_GpuPosDisplaced);
        ...
        public void OnDisable()
        {
            ...
            Gaussian4DSplatRenderSystem.instance.UnregisterSplat(this);
            m_Registered = false;
        ...
        internal void CalcViewData(CommandBuffer cmb, Camera cam)
        {
            ...
            cmb.SetComputeFloatParam(m_CSSplatUtilities, Props.MaxSplatSize, m_MaxSplatSize);
            cmb.SetComputeFloatParam(m_CSSplatUtilities, Props.MinSplatOpacity, m_MinSplatOpacity);
            cmb.SetComputeFloatParam(m_CSSplatUtilities, Props.TemporalCutoff, m_TemporalCutoff);
            cmb.SetComputeFloatParam(m_CSSplatUtilities, Props.Is4D, m_Is4D);
            cmb.SetComputeFloatParam(m_CSSplatUtilities, Props.SplatOpacityScale, m_OpacityScale);
            cmb.SetComputeIntParam(m_CSSplatUtilities, Props.SHOrder, m_SHOrder);
            cmb.SetComputeIntParam(m_CSSplatUtilities, Props.SHOnly, m_SHOnly ? 1 : 0);

            // re-pousser SplatCount + Is4D + temps JUSTE avant le dispatch (etat global du CS partage,
            // peut avoir ete change par le sort/un autre objet entre SetAssetDataOnCS et ici)
            cmb.SetComputeIntParam(m_CSSplatUtilities, Props.SplatCount, m_SplatCount);
            cmb.SetComputeFloatParam(m_CSSplatUtilities, Props.Is4D, m_Is4D);
            cmb.SetComputeFloatParam(m_CSSplatUtilities, Shader.PropertyToID("_SplatCurrentTime"), m_TemporalCurrentTime);
            // re-bind du buffer temporel JUSTE avant le dispatch : il differe entre 4D (vrai buffer) et 3D (dummy),
            // et le binding sur le kernel partage est ecrase par l'autre objet (cause du telescopage 4D-vs-3D)
            cmb.SetComputeBufferParam(m_CSSplatUtilities, (int)KernelIndices.CalcViewData, Shader.PropertyToID("_SplatTemporal"), EnsureTemporalBuffer());
            cmb.SetComputeBufferParam(m_CSSplatUtilities, (int)KernelIndices.CalcViewData, Props.SplatPosDisplaced, m_GpuPosDisplaced);
            m_CSSplatUtilities.GetKernelThreadGroupSizes((int)KernelIndices.CalcViewData, out uint gsX, out _, out _);
            cmb.DispatchCompute(m_CSSplatUtilities, (int)KernelIndices.CalcViewData, (m_GpuView.count + (int)gsX - 1)/(int)gsX, 1, 1);
            m_PosDisplacedReady = 1;
        }
        ...
        internal void SortPoints(CommandBuffer cmd, Camera cam, Matrix4x4 matrix)
        {
            ...
            cmd.SetComputeBufferParam(m_CSSplatUtilities, (int)KernelIndices.CalcDistances, Props.SplatPosDisplaced, m_GpuPosDisplaced);
            cmd.SetComputeFloatParam(m_CSSplatUtilities, Props.Is4D, m_Is4D);
            cmd.SetComputeFloatParam(m_CSSplatUtilities, Props.PosDisplacedReady, m_PosDisplacedReady);
        ...
        public void Update()
        {
            ...
            if (m_PrevAsset != m_Asset || m_PrevHash != curHash)
            {
                ...
                {
                    Debug.LogError($"{nameof(Gaussian4DSplatRenderer)} component is not set up correctly (Resource references are missing), or platform does not support compute shaders");
                }
            }
        }
        ...
        public void EditCopySplatsInto(Gaussian4DSplatRenderer dst, int copySrcStartIndex, int copyDstStartIndex, int copyCount)
        {
            EditCopySplats(
                dst.transform,
